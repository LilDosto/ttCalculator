...
namespace DotNetMatrix
{
	...
	public class LUDecomposition : System.Runtime.Serialization.ISerializable
	{
		...
		public LUDecomposition(GeneralMatrix A)
		{
			...
			for (int j = 0; j < n; j++)
			{
				...
				if (p != j)
				{
					for (int k = 0; k < n; k++)
					{
						decimal t = LU[p][k]; LU[p][k] = LU[j][k]; LU[j][k] = t;
					}
		...
		/// <summary>Return pivot permutation vector as a one-dimensional decimal array</summary>
		/// <returns>     (decimal) piv
		/// </returns>
		virtual public decimal[] decimalPivot
		{
			get
			{
				decimal[] vals = new decimal[m];
				for (int i = 0; i < m; i++)
				{
					vals[i] = (decimal) piv[i];
				}
				return vals;
			}
		}
